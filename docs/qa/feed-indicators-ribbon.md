...
- The feed uses React Native's native scrolling sticky-header behavior, with a fixed status-bar inset. The ribbon stays visible while the search field has focus. Tapping Home again returns to the top and refreshes. Feed notices scroll with content and are excluded from impression events.
- Comments uses the actual navigation-header height and keyboard-aware bottom spacing. The reply cancel target stays reachable for long excerpts.
