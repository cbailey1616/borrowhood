...
].map((item, index) => ({ type: 'listing', listingType: 'lend', isFree: true, isAvailable: true, isBorrowed: false, condition: 'good', visibility: 'neighborhood', pricePerDay: 0, timesBorrowed: 3 + index, pendingRequests: 0, maxBorrowDays: 14, minBorrowDays: 1, createdAt: new Date(Date.now() - (index + 1) * 3600000).toISOString(), ...item, user: item.owner, ownerId: item.owner.id, photos: [item.photoUrl] }));
...
].map((message, index) => ({ id: `demo-message-${index}`, ...message, isRead: true, reactions: [], createdAt: new Date(Date.now() - (6 - index) * 60000).toISOString() }));
