...
describe('PaymentMethodsScreen', () => {
  ...
  it('shows empty state', async () => { const S = require('../../src/screens/PaymentMethodsScreen').default; const { findByText } = render(<S navigation={mockNavigation} route={route} />); await findByText('No payment methods yet'); });
  ...
  it('shows default badge', async () => { api.getPaymentMethods.mockResolvedValue([{ id: 'pm-1', brand: 'visa', last4: '4242', expMonth: 12, expYear: 2027, isDefault: true }]); const S = require('../../src/screens/PaymentMethodsScreen').default; const { findAllByText } = render(<S navigation={mockNavigation} route={route} />); const matches = await findAllByText(/Default/i); expect(matches.length).toBeGreaterThan(0); });
