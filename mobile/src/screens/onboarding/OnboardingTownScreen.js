...
export default function OnboardingTownScreen() {
  ...
  const finish = async () => {
    if (busy) return;
    if (!city.trim() || !state.trim()) { setError('Enter your town and state to find nearby items.'); return; }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: COLORS.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[styles.content, { paddingTop: insets.top + 32, paddingBottom: insets.bottom + 24 }]}>
        ...
        <Text style={styles.body}>See what neighbors are sharing. You can invite friends or join a neighborhood later.</Text>
        ...
        <HapticPressable accessibilityRole="button" disabled={busy || locating} onPress={finish} style={styles.button}>
          {busy ? <ActivityIndicator color="white" /> : <Text style={styles.buttonText}>Explore nearby items</Text>}
        </HapticPressable>
