...
export function exchangeAction(transaction, userId) {
  ...
  if ((borrower || lender) && !transaction.actualPickupAt && !transaction.hasDispute && ['approved', 'paid'].includes(transaction.status)) return {
    label: 'Confirm pickup', method: 'confirmRentalPickup',
    confirmation: borrower ? 'Only confirm once you have received the item.' : 'Only confirm once you have handed the item to your neighbor.',
  };
